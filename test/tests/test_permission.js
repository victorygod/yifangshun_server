 ...
 * 导出函数
 */
async function runPermissionTests(externalTestUsers) {
  console.log('\n📋 6. 测试权限控制');
  ...
  await test('POST /api/prescription/update - 管理员访问应能修改处方', async () => {
    const { response, data } = await request('POST', '/api/prescription/update', {
      prescriptionId: 'non_existent_prescription_id',
      status: '待审核',
      name: '管理员修改',
      age: '30'
    }, {
    ...
    // 权限验证通过，但因为处方不存在会返回错误（code: 1, statusCode: 400）
    assert(response.statusCode !== 401, '不应返回401未授权');
    assert(response.statusCode !== 403, '不应返回403禁止');
    assertEquals(data.code, 1, '因处方不存在返回错误');
    assert(data.message.includes('不存在'), '应提示处方不存在');
